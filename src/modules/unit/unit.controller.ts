...
import { UnitResponseSchema, UnitWithIncludesResponseSchema } from './unit.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'title', 'description', 'keywords', 'price', 'availability', 'type', 'imageUrls', 'videoUrls', 'locationId', 'acceptedSellRequestId', 'deletedAt', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = ['title', 'description', 'keywords'];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['location', 'bookings', 'favorites', 'acceptedSellRequest'];
...
/**
 * Unit Controller
 * Handles HTTP request/response for the Unit resource.
 */
export class UnitController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => UnitResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      if (!query.filter.availability) {
        query.filter.availability = 'AVAILABLE';
      }
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? UnitWithIncludesResponseSchema.parse(item)
          : UnitResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `Unit with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? UnitWithIncludesResponseSchema.parse(result)
          : UnitResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, UnitResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, UnitResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
