...
import { GalleryItemResponseSchema, GalleryItemWithIncludesResponseSchema } from './galleryItem.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'title', 'description', 'details', 'keywords', 'type', 'imageUrls', 'videoUrls', 'deletedAt', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = ['title', 'description', 'details', 'keywords'];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['comments', 'reactions', 'favorites'];
 ...
 * Handles HTTP request/response for the GalleryItem resource.
 */
export class GalleryItemController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => GalleryItemResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? GalleryItemWithIncludesResponseSchema.parse(item)
          : GalleryItemResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `GalleryItem with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? GalleryItemWithIncludesResponseSchema.parse(result)
          : GalleryItemResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, GalleryItemResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, GalleryItemResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
