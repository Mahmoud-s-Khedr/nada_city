...
const { GalleryItemRepository } = await import('./galleryItem.repository.js');

const delegate = prismaMock.galleryItem;

describe('GalleryItemRepository', () => {
  let repo: InstanceType<typeof GalleryItemRepository>;

  beforeEach(() => {
    repo = new GalleryItemRepository();
    vi.clearAllMocks();
  });
  ...
  describe('toInclude', () => {
    ...
    it('normalizes valid include fields', () => {
      const include = { comments: true };
      const result = repo.toInclude(include);
      expect(result).toEqual({ comments: true });
    });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { title: 'test-title', description: 'test-description', details: 'test-details' };
