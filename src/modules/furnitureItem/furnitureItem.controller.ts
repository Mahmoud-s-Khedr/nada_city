...
import { FurnitureItemService } from './furnitureItem.service.js';
import { FurnitureItemResponseSchema, FurnitureItemWithIncludesResponseSchema } from './furnitureItem.dto.js';
import { buildQueryOptions } from '../../utils/query-builder.js';
import { sendSuccess, sendCreated, sendNoContent } from '../../utils/response.js';
import { ProblemDetail } from '../../middlewares/error.middleware.js';

const service = new FurnitureItemService();

/** Allowed fields for filtering and sorting — prevents querying on hidden/writeOnly fields. */
const ALLOWED_FILTER_FIELDS = ['id', 'title', 'description', 'price', 'imageUrls', 'videoUrls', 'deletedAt', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = ['title', 'description'];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['bookings', 'favorites'];
...
/**
 * FurnitureItem Controller
 * Handles HTTP request/response for the FurnitureItem resource.
 */
export class FurnitureItemController {
  ...
  /**
   * GET /api/v1/furnitureItems/cursor
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => FurnitureItemResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? FurnitureItemWithIncludesResponseSchema.parse(item)
          : FurnitureItemResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `FurnitureItem with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? FurnitureItemWithIncludesResponseSchema.parse(result)
          : FurnitureItemResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, FurnitureItemResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  /**
   * PATCH /api/v1/furnitureItems/:id
   */
  ...
  /**
   * DELETE /api/v1/furnitureItems/:id
   */
