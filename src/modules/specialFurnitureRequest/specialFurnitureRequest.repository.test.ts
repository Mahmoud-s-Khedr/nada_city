...
const { SpecialFurnitureRequestRepository } = await import('./specialFurnitureRequest.repository.js');

const delegate = prismaMock.specialFurnitureRequest;

describe('SpecialFurnitureRequestRepository', () => {
  let repo: InstanceType<typeof SpecialFurnitureRequestRepository>;

  beforeEach(() => {
    repo = new SpecialFurnitureRequestRepository();
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { name: 'test-name', phone: 'test-phone', details: 'test-details' };
