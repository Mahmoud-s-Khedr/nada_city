...
import { SpecialFurnitureRequestResponseSchema, SpecialFurnitureRequestWithIncludesResponseSchema } from './specialFurnitureRequest.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'userId', 'name', 'phone', 'details', 'status', 'adminNote', 'createdAt', 'updatedAt'];
...
/**
 * SpecialFurnitureRequest Controller
 * Handles HTTP request/response for the SpecialFurnitureRequest resource.
 */
export class SpecialFurnitureRequestController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => SpecialFurnitureRequestResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? SpecialFurnitureRequestWithIncludesResponseSchema.parse(item)
          : SpecialFurnitureRequestResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `SpecialFurnitureRequest with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? SpecialFurnitureRequestWithIncludesResponseSchema.parse(result)
          : SpecialFurnitureRequestResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, SpecialFurnitureRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, SpecialFurnitureRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
