...
const modelDelegate = prismaMock.whatsappOpenEvent;
...
function mockRecord(overrides: Record<string, unknown> = {}) {
  return {
    ...
    module: 'GALLERY',
    targetId: 'test-targetId-' + testId,
    defaultMessage: 'test-defaultMessage-' + testId,
...
describe('WhatsappOpenEvent API', () => {
  const basePath = '/api/v1/whatsappOpenEvents';
  const existingKey = {
    id: 'existing-id',
  };
  const missingKey = {
  ...
  describe('POST ' + basePath, () => {
    it('creates a new whatsappOpenEvent with mocked Prisma', async () => {
