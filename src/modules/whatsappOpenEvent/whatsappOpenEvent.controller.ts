...
import { WhatsappOpenEventService } from './whatsappOpenEvent.service.js';
import { WhatsappOpenEventResponseSchema, WhatsappOpenEventWithIncludesResponseSchema } from './whatsappOpenEvent.dto.js';
import { buildQueryOptions } from '../../utils/query-builder.js';
import { sendSuccess, sendCreated, sendNoContent } from '../../utils/response.js';
import { ProblemDetail } from '../../middlewares/error.middleware.js';

const service = new WhatsappOpenEventService();

/** Allowed fields for filtering and sorting — prevents querying on hidden/writeOnly fields. */
const ALLOWED_FILTER_FIELDS = ['id', 'userId', 'module', 'targetId', 'defaultMessage', 'createdAt'];
...
/**
 * WhatsappOpenEvent Controller
 * Handles HTTP request/response for the WhatsappOpenEvent resource.
 */
export class WhatsappOpenEventController {
  ...
  /**
   * GET /api/whatsappOpenEvents/cursor
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => WhatsappOpenEventResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? WhatsappOpenEventWithIncludesResponseSchema.parse(item)
          : WhatsappOpenEventResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `WhatsappOpenEvent with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? WhatsappOpenEventWithIncludesResponseSchema.parse(result)
          : WhatsappOpenEventResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, WhatsappOpenEventResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  /**
   * PATCH /api/whatsappOpenEvents/:id
   */
  ...
  /**
   * DELETE /api/whatsappOpenEvents/:id
   */
