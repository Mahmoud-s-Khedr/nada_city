...
const { WhatsappOpenEventRepository } = await import('./whatsappOpenEvent.repository.js');

const delegate = prismaMock.whatsappOpenEvent;

describe('WhatsappOpenEventRepository', () => {
  let repo: InstanceType<typeof WhatsappOpenEventRepository>;

  beforeEach(() => {
    repo = new WhatsappOpenEventRepository();
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { userId: 'test-userId', module: 'GALLERY', targetId: 'test-targetId' };
