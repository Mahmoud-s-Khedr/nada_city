...
import { FinishRequestResponseSchema, FinishRequestWithIncludesResponseSchema } from './finishRequest.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'userId', 'finishId', 'address', 'requestedAt', 'finishTypes', 'details', 'status', 'adminNote', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = [];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['user', 'finish'];
 ...
 * Handles HTTP request/response for the FinishRequest resource.
 */
export class FinishRequestController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => FinishRequestResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? FinishRequestWithIncludesResponseSchema.parse(item)
          : FinishRequestResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `FinishRequest with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? FinishRequestWithIncludesResponseSchema.parse(result)
          : FinishRequestResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, FinishRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, FinishRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
