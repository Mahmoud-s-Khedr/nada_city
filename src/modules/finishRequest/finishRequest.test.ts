...
const modelDelegate = prismaMock.finishRequest;
...
function mockRecord(overrides: Record<string, unknown> = {}) {
  return {
    ...
    finishId: 'test-finishId-' + testId,
    address: 'test-address-' + testId,
    requestedAt: new Date("2024-01-01T00:00:00.000Z").toISOString(),
    finishTypes: 'test-finishTypes-' + testId,
...
describe('FinishRequest API', () => {
  const basePath = '/api/v1/finishRequests';
  const existingKey = {
    id: 'existing-id',
  };
  const missingKey = {
  ...
  describe('POST ' + basePath, () => {
    it('creates a new finishRequest with mocked Prisma', async () => {
