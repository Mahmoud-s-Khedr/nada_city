...
import { ReactionResponseSchema, ReactionWithIncludesResponseSchema } from './reaction.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'type', 'galleryItemId', 'userId', 'createdAt'];
...
/**
 * Reaction Controller
 * Handles HTTP request/response for the Reaction resource.
 */
export class ReactionController {
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? ReactionWithIncludesResponseSchema.parse(item)
          : ReactionResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `Reaction with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? ReactionWithIncludesResponseSchema.parse(result)
          : ReactionResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, ReactionResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, ReactionResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
