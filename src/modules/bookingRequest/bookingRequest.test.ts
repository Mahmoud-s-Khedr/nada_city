...
const modelDelegate = prismaMock.bookingRequest;
...
function mockRecord(overrides: Record<string, unknown> = {}) {
  return {
    id: 'test-id-' + testId,
    unitId: 'test-unitId-' + testId,
    userId: 'test-userId-' + testId,
    name: 'test-name-' + testId,
    phone: 'test-phone-' + testId,
...
function validPayload() {
  return {
    unit: { create: { title: 'test-title-' + testId, description: 'test-description-' + testId, keywords: 'test-keywords-' + testId, price: 1.5, availability: 'AVAILABLE', type: 'RESIDENTIAL', imageUrls: 'test-imageUrls-' + testId, videoUrls: 'test-videoUrls-' + testId, locationId: 'test-locationId-' + testId, acceptedSellRequestId: 'test-acceptedSellRequestId-' + testId, deletedAt: new Date("2024-01-01T00:00:00.000Z").toISOString() } },
...
describe('BookingRequest API', () => {
  const basePath = '/api/v1/bookingRequests';
  const existingKey = {
    id: 'existing-id',
  };
  const missingKey = {
  ...
  describe('POST ' + basePath, () => {
    it('creates a new bookingRequest with mocked Prisma', async () => {
