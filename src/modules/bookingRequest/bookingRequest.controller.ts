...
import { BookingRequestResponseSchema, BookingRequestWithIncludesResponseSchema } from './bookingRequest.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'unitId', 'userId', 'name', 'phone', 'address', 'details', 'status', 'adminNote', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = [];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['unit', 'user'];
...
/**
 * BookingRequest Controller
 * Handles HTTP request/response for the BookingRequest resource.
 */
export class BookingRequestController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => BookingRequestResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? BookingRequestWithIncludesResponseSchema.parse(item)
          : BookingRequestResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `BookingRequest with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? BookingRequestWithIncludesResponseSchema.parse(result)
          : BookingRequestResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, BookingRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, BookingRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
