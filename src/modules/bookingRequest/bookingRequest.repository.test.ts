...
const { BookingRequestRepository } = await import('./bookingRequest.repository.js');

const delegate = prismaMock.bookingRequest;

describe('BookingRequestRepository', () => {
  let repo: InstanceType<typeof BookingRequestRepository>;
  ...
  describe('toInclude', () => {
    ...
    it('normalizes valid include fields', () => {
      ...
      expect(result).toEqual({ unit: true });
    });
