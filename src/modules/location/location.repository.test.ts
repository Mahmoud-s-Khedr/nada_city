...
const { LocationRepository } = await import('./location.repository.js');

const delegate = prismaMock.location;

describe('LocationRepository', () => {
  let repo: InstanceType<typeof LocationRepository>;
  ...
  describe('toInclude', () => {
    ...
    it('normalizes valid include fields', () => {
      const include = { units: true };
      const result = repo.toInclude(include);
      expect(result).toEqual({ units: true });
    });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { address: 'test-address', latitude: 1.5, longitude: 1.5 };
