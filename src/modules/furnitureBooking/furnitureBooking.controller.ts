...
import { FurnitureBookingResponseSchema, FurnitureBookingWithIncludesResponseSchema } from './furnitureBooking.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'furnitureItemId', 'userId', 'name', 'phone', 'address', 'details', 'status', 'adminNote', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = [];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['furnitureItem', 'user'];
...
/**
 * FurnitureBooking Controller
 * Handles HTTP request/response for the FurnitureBooking resource.
 */
export class FurnitureBookingController {
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => FurnitureBookingResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? FurnitureBookingWithIncludesResponseSchema.parse(item)
          : FurnitureBookingResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `FurnitureBooking with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? FurnitureBookingWithIncludesResponseSchema.parse(result)
          : FurnitureBookingResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, FurnitureBookingResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, FurnitureBookingResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
