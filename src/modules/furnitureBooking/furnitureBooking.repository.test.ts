...
const { FurnitureBookingRepository } = await import('./furnitureBooking.repository.js');

const delegate = prismaMock.furnitureBooking;

describe('FurnitureBookingRepository', () => {
  let repo: InstanceType<typeof FurnitureBookingRepository>;

  beforeEach(() => {
    repo = new FurnitureBookingRepository();
    vi.clearAllMocks();
  });
  ...
  describe('toInclude', () => {
    ...
    it('normalizes valid include fields', () => {
      const include = { furnitureItem: true };
      const result = repo.toInclude(include);
      expect(result).toEqual({ furnitureItem: true });
    });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { name: 'test-name', phone: 'test-phone', address: 'test-address' };
