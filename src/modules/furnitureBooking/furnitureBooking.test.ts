...
const modelDelegate = prismaMock.furnitureBooking;
...
function mockRecord(overrides: Record<string, unknown> = {}) {
  return {
    id: 'test-id-' + testId,
    furnitureItemId: 'test-furnitureItemId-' + testId,
...
function validPayload() {
  return {
    ...
    phone: 'test-phone-' + testId,
    address: 'test-address-' + testId,
    furnitureItem: { create: { title: 'test-title-' + testId, description: 'test-description-' + testId, price: 1.5, imageUrls: 'test-imageUrls-' + testId, videoUrls: 'test-videoUrls-' + testId, deletedAt: new Date("2024-01-01T00:00:00.000Z").toISOString() } },
...
describe('FurnitureBooking API', () => {
  const basePath = '/api/v1/furnitureBookings';
  const existingKey = {
    id: 'existing-id',
  };
  const missingKey = {
  ...
  describe('POST ' + basePath, () => {
    it('creates a new furnitureBooking with mocked Prisma', async () => {
