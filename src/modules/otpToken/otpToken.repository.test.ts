...
const { OtpTokenRepository } = await import('./otpToken.repository.js');

const delegate = prismaMock.otpToken;

describe('OtpTokenRepository', () => {
  let repo: InstanceType<typeof OtpTokenRepository>;

  beforeEach(() => {
    repo = new OtpTokenRepository();
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { email: 'test-email', code: 'test-code', consumed: true };
