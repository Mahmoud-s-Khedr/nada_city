...
import { OtpTokenService } from './otpToken.service.js';
import { OtpTokenResponseSchema, OtpTokenWithIncludesResponseSchema } from './otpToken.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'email', 'code', 'consumed', 'expiresAt', 'createdAt'];
...
/**
 * OtpToken Controller
 * Handles HTTP request/response for the OtpToken resource.
 */
export class OtpTokenController {
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? OtpTokenWithIncludesResponseSchema.parse(item)
          : OtpTokenResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `OtpToken with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? OtpTokenWithIncludesResponseSchema.parse(result)
          : OtpTokenResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, OtpTokenResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, OtpTokenResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
