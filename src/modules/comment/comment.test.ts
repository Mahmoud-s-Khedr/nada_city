...
const modelDelegate = prismaMock.comment;
...
function mockRecord(overrides: Record<string, unknown> = {}) {
  return {
    id: 'test-id-' + testId,
    body: 'test-body-' + testId,
    galleryItemId: 'test-galleryItemId-' + testId,
...
function validPayload() {
  return {
    body: 'test-body-' + testId,
    galleryItem: { create: { title: 'test-title-' + testId, description: 'test-description-' + testId, details: 'test-details-' + testId, keywords: 'test-keywords-' + testId, type: 'UNIT', imageUrls: 'test-imageUrls-' + testId, videoUrls: 'test-videoUrls-' + testId, deletedAt: new Date("2024-01-01T00:00:00.000Z").toISOString() } },
...
describe('Comment API', () => {
  const basePath = '/api/v1/comments';
  const existingKey = {
    id: 'existing-id',
  };
  const missingKey = {
  ...
  describe('POST ' + basePath, () => {
    it('creates a new comment with mocked Prisma', async () => {
