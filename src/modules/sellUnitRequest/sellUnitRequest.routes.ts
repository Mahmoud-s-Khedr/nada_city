...
router.get('/:id', authenticate, async (req, res, next) => {
  try {
    const item = await service.findOne({ id: String(req.params.id) }, { user: true, acceptedUnit: true });
    if (!item) {
...
router.post('/', authenticate, authorize('USER', 'ADMIN'), validate(CreateSellUnitRequestPublicSchema), async (req, res, next) => {
  try {
    const body = req.body as z.infer<typeof CreateSellUnitRequestPublicSchema>;
...
router.patch('/:id/review', authenticate, authorize('ADMIN'), validate(ReviewSellUnitRequestSchema), async (req, res, next) => {
  try {
    ...
    const result = await service.review({ id: String(req.params.id) }, body);
