...
const { SellUnitRequestRepository } = await import('./sellUnitRequest.repository.js');

const delegate = prismaMock.sellUnitRequest;

describe('SellUnitRequestRepository', () => {
  let repo: InstanceType<typeof SellUnitRequestRepository>;

  beforeEach(() => {
    repo = new SellUnitRequestRepository();
    vi.clearAllMocks();
  });
