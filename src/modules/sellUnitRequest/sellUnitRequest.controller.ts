...
import { SellUnitRequestService } from './sellUnitRequest.service.js';
import { SellUnitRequestResponseSchema, SellUnitRequestWithIncludesResponseSchema } from './sellUnitRequest.dto.js';
import { buildQueryOptions } from '../../utils/query-builder.js';
import { sendSuccess, sendCreated, sendNoContent } from '../../utils/response.js';
import { ProblemDetail } from '../../middlewares/error.middleware.js';

const service = new SellUnitRequestService();

/** Allowed fields for filtering and sorting — prevents querying on hidden/writeOnly fields. */
const ALLOWED_FILTER_FIELDS = ['id', 'userId', 'title', 'description', 'price', 'type', 'address', 'locationId', 'details', 'imageUrls', 'videoUrls', 'status', 'adminNote', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = ['title', 'description', 'address'];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['user', 'acceptedUnit'];
...
/**
 * SellUnitRequest Controller
 * Handles HTTP request/response for the SellUnitRequest resource.
 */
export class SellUnitRequestController {
  ...
  /**
   * GET /api/v1/sellUnitRequests/cursor
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => SellUnitRequestResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? SellUnitRequestWithIncludesResponseSchema.parse(item)
          : SellUnitRequestResponseSchema.parse(item)
      );
  ...
  /**
   * GET /api/v1/sellUnitRequests/:id
   */
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `SellUnitRequest with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? SellUnitRequestWithIncludesResponseSchema.parse(result)
          : SellUnitRequestResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  /**
   * PUT /api/v1/sellUnitRequests/:id
   */
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, SellUnitRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  /**
   * PATCH /api/v1/sellUnitRequests/:id
   */
  ...
  /**
   * DELETE /api/v1/sellUnitRequests/:id
   */
