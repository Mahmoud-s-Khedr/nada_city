...
const { UnitOrderRequestRepository } = await import('./unitOrderRequest.repository.js');

const delegate = prismaMock.unitOrderRequest;

describe('UnitOrderRequestRepository', () => {
  let repo: InstanceType<typeof UnitOrderRequestRepository>;

  beforeEach(() => {
    repo = new UnitOrderRequestRepository();
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('creates and returns a new record', async () => {
      const data = { title: 'test-title', description: 'test-description', minPrice: 1.5 };
