...
import { UnitOrderRequestResponseSchema, UnitOrderRequestWithIncludesResponseSchema } from './unitOrderRequest.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'userId', 'title', 'description', 'minPrice', 'maxPrice', 'type', 'address', 'location', 'details', 'status', 'adminNote', 'createdAt', 'updatedAt'];
const SEARCHABLE_FIELDS: string[] = ['title', 'description', 'address', 'location'];
const ALLOWED_INCLUDE_RELATIONS: string[] = ['user'];
...
/**
 * UnitOrderRequest Controller
 * Handles HTTP request/response for the UnitOrderRequest resource.
 */
export class UnitOrderRequestController {
  ...
  /**
   * GET /api/v1/unitOrderRequests/cursor
  ...
  async listCursor(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) => UnitOrderRequestResponseSchema.parse(item));
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? UnitOrderRequestWithIncludesResponseSchema.parse(item)
          : UnitOrderRequestResponseSchema.parse(item)
      );
  ...
  /**
   * GET /api/v1/unitOrderRequests/:id
   */
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `UnitOrderRequest with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? UnitOrderRequestWithIncludesResponseSchema.parse(result)
          : UnitOrderRequestResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  /**
   * PUT /api/v1/unitOrderRequests/:id
   */
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, UnitOrderRequestResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  /**
   * PATCH /api/v1/unitOrderRequests/:id
   */
  ...
  /**
   * DELETE /api/v1/unitOrderRequests/:id
   */
