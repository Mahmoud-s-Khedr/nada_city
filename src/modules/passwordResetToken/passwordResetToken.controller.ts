...
import { PasswordResetTokenResponseSchema, PasswordResetTokenWithIncludesResponseSchema } from './passwordResetToken.dto.js';
...
const ALLOWED_FILTER_FIELDS = ['id', 'email', 'token', 'consumed', 'expiresAt', 'createdAt'];
const SEARCHABLE_FIELDS: string[] = [];
const ALLOWED_INCLUDE_RELATIONS: string[] = [];
 ...
 * Handles HTTP request/response for the PasswordResetToken resource.
 */
export class PasswordResetTokenController {
  ...
  async list(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      const safeData = data.map((item: unknown) =>
        hasInclude
          ? PasswordResetTokenWithIncludesResponseSchema.parse(item)
          : PasswordResetTokenResponseSchema.parse(item)
      );
    ...
    try {
      ...
      if (!result) {
        throw new ProblemDetail({
          ...
          title: 'Not Found',
          status: 404,
          detail: `PasswordResetToken with key "${this.keyDescription(req)}" not found.`,
        });
      }
      const hasInclude = !!options.include && Object.keys(options.include).length > 0;
      sendSuccess(
        res,
        hasInclude
          ? PasswordResetTokenWithIncludesResponseSchema.parse(result)
          : PasswordResetTokenResponseSchema.parse(result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async create(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendCreated(res, PasswordResetTokenResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ControllerRequest, res: ControllerResponse, next: ControllerNext): Promise<void> {
    try {
      ...
      sendSuccess(res, PasswordResetTokenResponseSchema.parse(result));
    } catch (error) {
      next(error);
    }
  }
