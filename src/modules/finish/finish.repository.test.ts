...
const { FinishRepository } = await import('./finish.repository.js');

const delegate = prismaMock.finish;

describe('FinishRepository', () => {
  let repo: InstanceType<typeof FinishRepository>;

  beforeEach(() => {
    repo = new FinishRepository();
    vi.clearAllMocks();
  });
  ...
  describe('toInclude', () => {
    ...
    it('normalizes valid include fields', () => {
      const include = { requests: true };
      const result = repo.toInclude(include);
      expect(result).toEqual({ requests: true });
    });
